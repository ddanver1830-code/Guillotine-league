...
-- Skipped picks are represented in draft_picks so every existing Draft Room
-- screen automatically advances to the next snake-order pick.

create or replace function public.skip_expired_draft_pick(p_league_id uuid)
returns public.draft_state
language plpgsql security definer set search_path=public as $$
declare
  v public.draft_state; v_picks int; v_pick_number int; v_round int;
  v_team_number int; v_team_id uuid; v_total int; v_elapsed int;
begin
  ...
  if not v.timeout_pending and v_elapsed < coalesce(v.remaining_seconds,v.pick_seconds) then raise exception 'The pick clock has not expired.'; end if;
  ...
  insert into public.draft_picks(league_id,round,pick_number,team_id,sleeper_id,player_name,position)
    values(p_league_id,v_round,v_pick_number,v_team_id,null,'TIME EXPIRED — PICK FORFEITED','—');
  ...
  insert into public.audit_log(league_id,actor_user_id,action,details)
    values(p_league_id,auth.uid(),'draft_pick_timeout',jsonb_build_object('pick_number',v_pick_number,'team_id',v_team_id,'team_number',v_team_number,'action','forfeit'));
  return v;
exception when unique_violation then raise exception 'That expired pick has already been advanced. Refresh the Draft Control page.';
end; $$;
