-- Follow-up migration for draft_timeout.sql.
-- IMPORTANT: Run this file AFTER supabase/draft_timeout.sql.
-- The first migration is already installed. This file replaces its two draft RPCs
-- with the corrected versions below.
--
-- A forfeited pick is stored in draft_picks with sleeper_id = NULL. This means
-- the existing Draft Room (which uses draft_picks.length for snake order) moves
...
begin
  ...
  if not v.timeout_pending
     and v_elapsed < coalesce(v.remaining_seconds,v.pick_seconds) then
  ...
  if mod(v_round,2)=1 then
    v_team_number := mod(v_pick_number-1,18)+1;
  else
    v_team_number := 18-mod(v_pick_number-1,18);
  end if;
  ...
  if v_team_id is null then
    raise exception 'Expected Team % was not found.',v_team_number;
  end if;

  -- Record the forfeited pick in draft_picks so all existing draft-room
  -- screens advance to the next pick without needing a separate skip count.
  insert into public.draft_picks(
    league_id, round, pick_number, team_id, sleeper_id,
    player_name, position
  ) values (
    p_league_id, v_round, v_pick_number, v_team_id, null,
    'TIME EXPIRED — PICK FORFEITED', '—'
  );

  insert into public.draft_skips(
    league_id,pick_number,round,team_id,reason
  ) values (
    p_league_id,v_pick_number,v_round,v_team_id,'Pick clock expired'
  )
...
begin
  ...
  if v_ds.timeout_pending then
    raise exception 'The pick clock has expired. Commissioner action is required.';
  end if;
