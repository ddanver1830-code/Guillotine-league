...
begin
  ...
  select count(*) into v_locked
    ...
    where l.league_id=p_league_id and l.week=v_week and l.locked and t.alive;
  ...
  order by ws.points asc, v_low_tiebreak asc, t.team_number asc
  limit 1;
  ...
  return jsonb_build_object(
    ...
    'teams_alive',v_alive,
    'scores_calculated',v_scores,
    'lineups_locked',v_locked,
    'eliminated',v_elim,
    'pending_bids',v_pending,
    'lowest_team_id',v_low_team,
    'lowest_score',v_low_score,
    'lowest_season_points',v_low_tiebreak,
    'ready_for_elimination',(v_scores>0 and not v_elim),
    'ready_for_waivers',(v_elim and v_pending=0),
    'ready_to_advance',(v_elim and v_pending=0),
    'complete',(v_league.status='complete')
  );
end;
$$;
...
begin
  ...
  update public.lineups l
   ...
   where l.league_id=p_league_id
     and l.week=p_week
     and exists (select 1 from public.teams t where t.id=l.team_id and t.league_id=p_league_id and t.alive);
  get diagnostics v_count = row_count;
  insert into public.audit_log(league_id,actor_user_id,action,details)
  values(p_league_id,v_user,'lock_week_lineups',jsonb_build_object('week',p_week,'rows_locked',v_count));
  return v_count;
end;
$$;
...
begin
  ...
  if v_week >= 17 or v_alive <= 1 then
    select manager_name into v_champion from public.teams where league_id=p_league_id and alive limit 1;
    update public.leagues set status='complete' where id=p_league_id;
    insert into public.audit_log(league_id,actor_user_id,action,details)
    values(p_league_id,v_user,'league_complete',jsonb_build_object('week',v_week,'champion',v_champion));
  ...
  insert into public.audit_log(league_id,actor_user_id,action,details)
  values(p_league_id,v_user,'advance_week',jsonb_build_object('from_week',v_week,'to_week',v_week+1));
